...
async def check_session_url(session_url):
    ...
    try:
        async with session.get(session_url, allow_redirects=True, headers=headers) as response:
            ...
            # URL စစ်ဆေးမှုကို ပိုမိုလျှော့ပေါ့လိုက်သည်
            if response.status in (200, 301, 302) or "sessionId" in final_url or "sessionId" in body:
                return True
            return False
    except Exception as e:
...
async def Code_Expires_Date(session_id):
    ...
    try:
        ...
        async with aiohttp.ClientSession(
            ...
            # HTTPS သို့ ပြောင်းလဲခြင်းနှင့် Response ကို ပိုမိုသေချာစွာ စစ်ဆေးခြင်း
            async with fresh_session.get(
                ...
                # အကယ်၍ API က အချက်အလက် မပေးနိုင်လျှင် အနည်းဆုံး Status code ကို ပြပေးရန်
                return f"📋 Plan: Unknown | ⏳ Time: Unknown (Status: {req.status})"
    except Exception as e:
