...
def init_db():
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    # အစ်ကို့ Log ထဲတွင် ပါဝင်သော auth_keys Table နှင့် keys Table နှစ်ခုလုံးကို အော်တိုတည်ဆောက်ပေးခြင်း
...
if __name__ == "__main__":
    # Flask Web ဆာဗာကို အရင်ဦးစွာ သီးသန့် ဖွင့်လှစ်ခြင်း
    t = threading.Thread(target=run_web_server)
    t.daemon = True
    t.start()
    
    # ပတ်လည် မောင်းနှင်မည့် Bot Polling ကို စနစ်တကျ သီးသန့်စာကြောင်းဖြင့် Run ခြင်း
    print(f"[+] Render Auto-Restore Bot is running...")
    bot.infinity_polling()    if not GITHUB_TOKEN:
        print("[-] Pull Error: GH_TOKEN is not set!")
        return
